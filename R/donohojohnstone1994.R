...
plot(t, blocks(t), type = "l")

bumps_ <- function(x, tj, hj, wj) {
...
## Ratkovic
##
ratkovic_jump <- function(x, nu = 1) {
  f <- function(x) besselJ(x, nu)
  -2 * f(x / 100) + 8 * (x > 200) - 4 * (x > 500) + 2 * (x > 800)
}
ratkovic_nojump <- function(x, nu = 1) {
  f <- function(x) besselJ(x, nu)
  -2 * f(x / 100)
}
x <- 1:1000
plot(x, ratkovic_jump(x))
plot(x, ratkovic_nojump(x))
