...
m0_mu_summary <-
  nile_df %>%
  mutate(mean = mean(m0_mu),
         p025 = quantile(m0_mu, prob = 0.025),
         p975 = quantile(m0_mu, prob = 0.975))

ggplot(m0_mu_summary, aes(x = year)) + 
...
```{r}
waic2df <- function(x) {
  as_data_frame(x[names(x)[! names(x) %in% c("pointwise")]])
}

list(mutate(waic2df(waic(rstan::extract(m0_ret, "log_lik")[[1]])),
            model = "Constant"),
     mutate(waic2df(waic(t(rstan::extract(m1_ret, "log_lik")[[1]]))),
            model = "Intervention"),
      mutate(waic2df(waic(rstan::extract(m2_ret, "log_lik")[[1]])),
            model = "Normal"),
      mutate(waic2df(waic(rstan::extract(m3_ret, "log_lik")[[1]])),
            model = "Student t"),
      mutate(waic2df(waic(rstan::extract(m4_ret, "log_lik")[[1]])),
            model = "Laplace"),
      mutate(waic2df(waic(rstan::extract(m5_ret, "log_lik")[[1]])),
            model = "Horseshoe"),
      mutate(waic2df(waic(rstan::extract(m6_ret, "log_lik")[[1]])),
            model = "Horseshoe+")) %>%
  bind_rows()
