...
The sparse state disturbances can also be extended to multivariate state spaces.
In the case of a state space $|\eta_{t}| > 1$, the state covariance matrix is
\begin{equation}
  \label{eq:12}
  \eta_{t} & \sim \dmvnorm(\Lambda_{t} \Sigma \Lambda_{t}'){m}
\end{equation}
$\Sigma$ is the global scale covariance matrix.
$\Lambda_{t}$ is the local scale covariance matrix.
