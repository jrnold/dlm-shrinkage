...
\begin{equation}
  ...
  \begin{aligned}[t]
    ...
    \lambda_{t} & \dhalfcauchy{0, 1}
  \end{aligned}
\end{equation}
The model is completed with non-informative priors on $\sigma^{2}$ and $\tau$ as in \textcite{CarvalhoPolsonScott2010}{DattaGhosh2012},
\begin{align}
  \label{eq:9}
  \begin{aligned}[t]
...
\begin{equation}
  \label{eq:10}
  E(\omega_{t} | \theta_{t - 1}, \sigma, \tau, \lambda_{t}) =
  \left(
...
Figure \ref{fig:nile} plots the original data, and the mean of the posterior predicted data of the horseshoe prior model and the dynamic linear model.
...
\begin{figure}[htpb]
  \centering
  \includegraphics{plots/fig-cp6.pdf}
  \caption{Plot of mean posterior predictive distributions ($\E p(\tilde{y}| y)$) for the normal, normal2, and horseshoe prior distribution models.}
  \label{fig:cp6}
\end{figure}

Figure \ref{fig:cp6_w} plots the value of the probability of the structural break, as described in section ???.
As expected, only the year 1899 has a structural break probability greater than 0.5.

\begin{figure}[htpb]
  \centering
  \includegraphics{plots/fig-cp6_w.pdf}
  \caption{Plot of estimated probability of a structural break, calculated as $w_{i} = 1 - \E(\hat{\kappa})$}
  \label{fig:cp6_w}
\end{figure}
...
\begin{figure}[htpb]
  \centering
  \includegraphics{plots/fig-cp6_innovations.pdf}
  \caption{Plot of innovations}
  \label{fig:cp6_innovations}
\end{figure}

\begin{table}[htpb]
  \centering
  \input{plots/tab-cp6.tex}
  \caption{Model summary statistics of the cp6 models.}
  \label{tab:cp6}
\end{table}
