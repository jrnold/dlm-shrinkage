...
\begin{abstract}
  This paper shows that structural breaks can be estimated in a Bayesian continuous state space approach with dynamic linear models by placing shrinkage priors on the distribution of the per-period differences in parameters.
  This model nests many common structural break and smoothing time-varying parameter models.
  This model does not require a specification of the number of structural breaks, and is flexible.
  An efficient method to estimate these models in Stan is presented.
  Applications of this method to several problems are presented.
\end{abstract}
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
The difference between "structural breaks" and "smoothing" data-generating processes and estimation techniques is whether the $\eta$ vector is be sparse (most $\eta_{t} = 0$) or dense (most $\eta_{t} \neq 0$).
It is common to estimate to apply a normal distribution to $\eta_{t} \sim N(0, Q)$. 
...
The reason for seperating the steps is that directly sampling $\alpha$ seems to cause problems for the HMC, while the log-likelihood of $p(y_{t} | S_{t})$ can be easily and efficiently calculated.
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
The third model, $M_{nile,inter}$ represents an ``indeal'' intervention to account for a structural break in 1899.
...
Figure \ref{fig:nile-posterior} plots the posterior distributions of the level ($p(\alpha | y)$) for the three models against the data.
...
In other words, the normal model oversmooths at the break, and undersmooths in other periods.
...
A global-local scale mixture of normal distribution like the horseshoe prior is able to accomodate the break.
$\tau$ estimates the overall sparsity of $\eta_{t}$, while the individual values of $\lambda_{t}$ allow for signals.
Surpisingly, the horseshoe prior model produces constant estimates of the level before and after 1899, while the manual intervention model shows slight changes.
...
\footnote{See \textcite{GelmanHwangVehtari2013} and \textcite{VehtariOjanen2012} for recent overviews of Byaesian model comparison.}
...
\begin{figure}[htpb]
  ...
  \begin{subfigure}{1.0\textwidth}
    \includegraphics{plots/fig-nile1}
    \caption{$M_{nile,hs}$}
    \label{fig:nile-posterior-1}    
  \end{subfigure}
  \begin{subfigure}{1.0\textwidth}
    \includegraphics{plots/fig-nile2}
    \caption{$M_{nile,normal3}$}
    \label{fig:nile-posterior-2}
  \end{subfigure}
  \begin{subfigure}{1.0\textwidth}
    \includegraphics{plots/fig-nile3}
...
\begin{figure}[htpb]
  \centering
  \includegraphics{plots/fig-nile-eta}
...
\begin{figure}[htpb]
  \centering
  \includegraphics{plots/fig-nile-lambda}
...
\begin{singlespace}
  \include{plots/code-local_level_hp}  
\end{singlespace}
