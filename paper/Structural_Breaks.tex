...
For now, I will restrict my attention to the case of a single univariate observation vector $y_{1:n}$, with a time-varying mean.
...
Note that given an initial $\alpha_{1}$, changes in the level of $y_{t}$ are determined by the random variable $\eta_{t}$, and thus its distribution.
Thus the values of $Q_{t}$ are going to determine how much the parameter can change per period, i.e. $\alpha_{t + 1} - \alpha_{t}$
Also note that if $H$ and $Q$ are to be estimated, there are more parameters than observations. 
...
However, assuming $Q_{t} = Q$ for all time periods will not work for many political science problems and data.
Many political processes, namely those in which the researcher believes there may  structural breaks, are characterized by long spells of stability and a few periods of dramatic change.
...
Recasting the problem in this way, shows that the time-varying parameters problem is 
The problem with using $\eta_{t} \sim dnorm(0, Q)$ if the true $\eta$ are sparse.
