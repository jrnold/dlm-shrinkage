...
Structural breaks and smoothing approaches to estimating time-varying parameters are often thought of as distinct and estimated with different methods.
This paper shows that both structural break methods and smoothing approaches can both be nested within continuous state space models and in particular 
The fundamental difference between structural break models and smoothing approaches is in the assumptions they make about the distribution of the parameter innovations (changes).
...
However, it is important to note that both of these models can be represented in a continuous state space model, and just differ in the distributions placed on the innovations.
...
Not only can structural breaks be modeled within a continuous state space approach, structural breaks can be modeled within a 
...
A barrier to estimation is that there are often only $T$ data points, meaning that some additional structure must be applied to estimate 
The time-varying parameter problem is distinct from other large-p, small-n problems due to the additional structure implied by an ordering of the parameters by time.
Many approaches to estimating time-varying parameters can be thought of as implying different restrictions on per-period difference in the parameters $\Delta \theta_{t} = \theta_{t} - \theta_{t - 1}$.%
Smoothing and structural break approaches to estimating time-varying parameters are analagous to shrinkage and variable selection approaches.
