...
Consider the case of multiple means.
...
In equation \eqref{eq:5}, the first difference of the state is equal to the value of the state disturbance, $\Delta \theta_{t} = \theta_{t+1} - \theta_{t} = \omega_{t}$.
Thus, the distribution of parameter differences is the distribution of $\omega_{t}$.
...
Translated into equation \eqref{eq:5}, this statement means that the researcher has a prior belief that the vector $\omega$ is likely sparse.
I will refer to a vector as sparse if most elements are 0, and dense if most elements are not zero.
It is expected that for most periods $\omega_{t} = 0$ or $\omega_{t} \approx 0$, but there are a few periods in which $\omega_{t} \neq 0$, with the possibility that $|\omega_{t}| \gg 0$.
When cast in this way, the problem of estimating structural breaks within a dynamic linear model is essentially a problem of estimating a sparse parameter vector $\omega$.
...
Following the convention in that literature, I will refer $\omega_{t} \approx 0$ as ``noise'' and $|\omega_{t}| \gg 0$ as ``signals''.
...
\begin{align}
  \label{eq:18}
  \mu_{t} | y_{1:t-1} & \sim N(m_{t-1}, R_{t} = C_{t - 1} + W) \\
  \omega_{t} | y_{1:t-1} & \sim N(m_{t-1}, R_{t} = C_{t - 1} + W) \\
  \mu_{t} | y_{1:t} & \sim N(m_{t} = m_{t-1} + A_{t} e_{t}, C_{t} = A_{t} V)
\end{align}

Then $A_{t} = R_{t} / Q_{t} = \frac{C_{t-1} + W_{t}}{C_{t-1} + W + V}$, where $A_{t} \in [0, 1]$.
This is influenced by the signal to noise ratio $r = \frac{W}{V}$.
Then $m_{t} = C_{t} (m_{t-1} / R_{t} + Y_{t} / V_{t}) = A_{t} Y_{t} + (1 - A_{t}) m_{t - 1}$.
Thus $m_{t}$ is a weighted average of the prior estimate $m_{t-1}$ and the observation $Y_{t}$.
When $R_{t} < V_{t}$, the prior distribution is more concentrated than the observation variance and $A_{t}$ is closer to 0.
...
In the context of estimating TVP, structural breaks are estimated assigning each $\omega_{t}$ a discrete mixture distribution,
\begin{equation}
  \label{eq:1}
  \omega_{t} = w \delta_{0} +  (1 - w) g(\omega_{t})
\end{equation}
where $w \in [0, 1]$, $\delta_{0}$ is the degenerate distribution at 0, and $g(\omega_{t})$ is the distribution if $\omega_{t} \neq 0$.
...
\footnote{
Note that the commonly used $\omega_{t} \sim N(0, Q)$ is trivially a scale-mixture of normal distributions ($\lambda_{t}^{2} = \delta_{1}$).
...
As discussed in more delta in section \ref{sec:implementation}, this means that the dynamic linear model component of the model can be sampled using efficient block sampling methods that depend on $\omega_{t}$ and $\varepsilon_{t}$ being distributed normal.
...
\footnote{
The half-Cauchy prior on the scale components $\lambda_{t}$ implies an inverse-Bomega (Bomega prime) distribution on the variance components $\lambda_{t}^{2} = \dinvbomega{\frac{1}{2}, \frac{1}{2}}$, where $\dinvbomega{x; a, b} = \frac{x^{a - 1} (1 + x)^{-a - b}}{B(a, b)}$, where $B$ is the Bomega function.
The inverse bomega distribution is related to the bomega distribution.
If $X \sim \dbomega{a, b}$, then $\frac{1}{1 + X} \sim \dinvbomega{a, b}$.
}
\begin{equation}
  \label{eq:6}
...
Using shrinkage priors to model structural breaks extends and generalizes the use of the $t$-distribution for modeling structural breaks \parencites{HarveyKoopman2000}[184][]{DurbinKoopman2001}{PetrisPetroneOmegal2009}.
