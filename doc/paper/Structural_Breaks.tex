...
Consider the case of multiple testing of means,%
\footnote{This easily extends to linear regression $y_{i} \sim N(\theta_{i} X_{i}, \sigma^{2})$},
 where
\begin{equation}
  \label{eq:13}
  y_{i} & \sim N(\theta_{i}, \sigma) & & \text{for $i \in 1:n$}}
\end{equation}
where the vector $\theta_{1:n}$ is sparse, meaning that it is expected that most $\theta_{i} = 0$.
...
\textit{advantages of this model: seperates inclusion and size of signal, w as a test of nonzero. disadvantages: hard to estimate. exact 0s}
...
The most popular class of Bayesian shrinkage priors are global-local mixtures of normal distributions \parencite{PolsonScott2010}.
A global-local mixture of normal distributions 
\begin{equation}
  \label{eq:3}
  \begin{aligned}[t]
...
If $g$ is sufficiently heavy-tailed, $\E(\theta_{i} | w, y_{i}) \approx w(y_{i}) y_{i}$. 

The analog of $w$ in the global-local normal distributions is the shrinkage parameter $\kappa$.
Following standard results, the posterior distribution of $\theta_{i}$ is,
\begin{equation}
  \label{eq:11}
...
The  distributions of $\kappa$, since $p(\lambda)$ imply prior distributions of $p(\tau$). 
Discuss prior distributions over kappa.

While there are a ``thicket'' of global-local shrinkage distributions, and the properties of these are still being analyzed, this paper will make use of the horseshoe prior distribution.
\textit{discuss properties and why I use it.}
...
Now consider estimation of time-varying means using a local level model,
\begin{equation}
  \label{eq:5}
  \begin{aligned}[t]
    y_{t} &= \mu_{t} + \nu_{t} & \nu_{t} & \sim \dnorm{0, V} \\
...
Since all the component equations in equation \eqref{eq:5} are normal, there are analytic forms of the updating equations.
\begin{align}
  \label{eq:18}
...
For the purposes of this paper, the important feature of the sequential updating is that the filtered estimate of $\theta_{t}$ is a weighted sum of the prior estiamte $m_{t - 1}$ and the forecast error $e_{t} = y_{t} - m_{t - 1}$ with weight $A_{t}$. 
This could also be rewritten as a weighted sum of $y_{t}$ and $m_{t-1}$, $\theta_{t} | y_{1:t} = (1 - A_{t}) m_{t-1} + A_{t} y_{t}$.
The weight is a function of the observation and system variances, $A_{t} = R_{t} / Q_{t} = \frac{C_{t-1} + W_{t}}{C_{t-1} + W + V}$.

There are three approaches to shrinkage in the local level model, differing in what quantity they put the prior distribution over.
\begin{enumerate}
\item Prior on $A_{t}$
\item Prior on $\delta_{t}$ 
\item Prior of $W_{t}$
\end{enumerate}
