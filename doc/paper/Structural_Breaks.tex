...
For this paper, I used the \RLang package \textit{KFAS} \parencite{Helske2012} which has an implementation the \textcite{DurbinKoopman2002} simulation smoother algorithm.
...
In the first model, \Model{nile}{hp}, $\eta_{t}$ has a horsehose prior,
\begin{equation}
  \label{eq:22}
  \begin{aligned}[t]
...
\begin{equation}
  ...
  \end{aligned}
\end{equation}
\todo[inline]{Change to multiplicative intervention so it is exactly analagous to the SDDLM.}
...
On the other hand, a global-local scale mixture like the horseshoe prior has two parameters which can hanle
...
Figure \ref{fig:nile-eta} displays summaries of the poterior distributions of the the state disturbances $p(\eta_{t} | y)$.
...
While this may seem like an easy application, it is an exaple with a sparse disturbance vector (expected 1 of 100), the domain in which continuous state space models have traditionally captured time-varying-parameters poorly (e.g. see \Model{nile}{normal}.).
However, shows that scale mixture of normal distributions can recover the structural break and sqeulch changes in the other periods.
