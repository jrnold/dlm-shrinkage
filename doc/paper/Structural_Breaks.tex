...
Consider a sequence of data generated from time varying means,
\footnote{This easily extends to linear regression $y_{i} \sim N(\theta_{i} X_{i}, \sigma^{2})$.}, where
\begin{align}
  \label{eq:13}
  y_{i} & \sim N(\theta_{i}, \sigma) & & \text{for $i \in 1:n$}
\end{align}
where the vector $\theta_{1:n}$ is sparse, meaning that most $\theta_{i} = 0$.
In Bayesian estimation, there are two main approaches for estimating a vector sparse parameters: selection using discrete mixtures and shrinkage using continuous priors \parencite[73]{CarvalhoPolsonScott2009}. 
...
The shrinkage approaches estimates sparsity using a continuous prior distribution centered at zero \parencites{Tibshirani1996}{Tipping2001}.
Most proposed Bayesian shrinkage priors in the the class of global-local mixtures of normal distributions \parencite{PolsonScott2010}:
\begin{equation}
  \label{eq:3}
  \begin{aligned}[t]
...
If $X \sim \dbomega{a, b}$, then $\frac{1}{1 + X} \sim \dinvbeta{a, b}$.
}
\begin{equation}
  \label{eq:6}
...
In \eqref{eq:13}, the data were not ordered in time. 
...
\begin{equation}
  \label{eq:28}
  p(y_{1:t}) = \prod p(y_{1} | y_{t - 1}) = \prod \dnorm{y_{t}; f_{t}, Q_{t}}
\end{equation}
and the log-likelihood is,
\begin{equation}
  \label{eq:29}
  l = - \frac{1}{2} \sum \log | Q_{t} | - \frac{1}{2} e_{t}'
\end{equation}

The estimation of $\theta_{1:T} | y_{1:T}$ is known as \textit{smoothing}; the smoothing distribution is calculated or samples from by first filtering forward from 1 to $T$, and then calculting or sampling from distributions backwards in time from $T$ to 1.
...
In this paper, I consider two approaches to priors that encourage sparse $A_{1:T}$: setting a prior on $W_{1:T}$ and using the standard Kalman filter, and setting a prior on $\delta_{1:T} = 1 - A_{1:T}$ and using the discounting filter.

In a discounting filter, the values of $W_{t}$ are modeled as a discounted value of the prior filtered variance $C_{t-1}$.
Let $R_{t} = C_{t - 1} + W_{t}$.
Thus $C_{t-1}$, is the variance of the state if there were no system error $W_{t}$, and the state does not change over time.

Note that in \eqref{eq:25},   
