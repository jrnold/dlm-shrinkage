...
Let $\Delta \alpha_{t} = \alpha_{t} - \alpha_{t - 1}$ and $e_{t} = y_{t} - \alpha_{t - 1}$, 
For the discrete mixture model in equation \eqref{eq:1}, if $g$ is sufficiently heavy-tailed, the posterior mean is $E(\Delta \alpha_{t}) \approx p_{t} e_{t} $, where $p_{t}$ is the posterior inclusion probability of $\eta_{t}$.
